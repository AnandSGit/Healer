...
**Reference `${CLAUDE_PLUGIN_ROOT}/shared/_enforcement.md` → Stack Auto-Detection Protocol.** Run it once, cache results. This determines:
...
Follow `${CLAUDE_PLUGIN_ROOT}/shared/_enforcement.md` → Research Protocol. Before writing docs, research documentation excellence using ACTUAL tools:
...
All values MUST come from actual verification. Never use placeholders. Follow `${CLAUDE_PLUGIN_ROOT}/shared/_enforcement.md` → Verification Protocol.
...
9. **Evidence before assertions** — follow `${CLAUDE_PLUGIN_ROOT}/shared/_enforcement.md` verification protocol for all claims
